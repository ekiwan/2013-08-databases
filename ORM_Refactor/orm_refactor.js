...
var Messages = sequelize.define('messages', {
  //M_id: {ty: Sequelize.INTEGER, autoIncrement: true},
...
Users.sync();
Messages.sync();
sequelize.sync("chat", "root");



exports.insertData = function(username, room, message) {
  console.log('were in insert messages');
  Messages.create({username: username, room: room, body: message}).success(function(message){
...
exports.addFriend = function(request, response) {
  ...
  request.on('end', function(err, data) {
    Users.find({where: {username: username1}}).success(function(user){
      console.log('user!!!!!', user);
      user.setFriends([username5]);
    });
  ...
  // var newUser = Users.create({username: "Jean Valjean"});
